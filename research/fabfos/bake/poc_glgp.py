"""The reaction the concern was raised about, priced three ways.

MNXR145036 is glycogen phosphorylase. MetaNetX writes it `G1P = Glycogen + Pi`, r9's
substitution lane restages it as `alpha-maltotriose + G1P = alpha-maltotetraose + Pi`, and
the bake ships ratio 0.204 -- polymer synthesis favoured about 5:1, which is backwards from
what glgP and malP do.

Nothing about the number is wrong as thermodynamics. dG'o = -4.18 kJ/mol is the standard
...
because phosphoglucomutase drains G1P as fast as it appears. That is a Q term, and the bake
has no Q term.
...
print("  as r9 restages it     :  " + " = ".join(
    " + ".join(f"{abs(c):g} {names.get(k, k)}" for k, c in rewritten.items()
               if (c < 0) == (s == 0))
    for s in (0, 1)))
print(f"\n  deployed: dG_raw {row.dG_raw:+.3f}  dG_prime {row.dG_prime:+.3f}  "
      f"ratio {row.ratio:.4f}  tier {row.dir_tier} {row.dir_method}  "
      f"lambda {row.lambda_shrink:.3f}")
...
dg_conc = row.dG_raw + corr
print(f"\n  correction from the measured pair alone : {corr:+.3f} kJ/mol "
      f"({corr / DECADE:+.2f} decades)")
print(f"  dG'  standard state                    : {row.dG_raw:+.3f} kJ/mol   "
      f"ratio {math.exp(row.dG_raw / RT):.4f}")
print(f"  dG'  with measured [Pi] and [G1P]      : {dg_conc:+.3f} kJ/mol   "
      f"ratio {math.exp(dg_conc / RT):.4f}")
print(f"\n  the ratio moves by a factor of {math.exp(corr / RT):.1f}")
pi = conc["MNXM9"][0]
