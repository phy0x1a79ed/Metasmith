# Pratama's binning (reproduction_map A5), CONCOCT alone. See metawrap_metabat2_pratama.py for why each
...
bins = model.AddProduct(lib.GetType("e3::metawrap_concoct_bins"))
...
TransformInstance(
    ...
    model=model,
    group_by=asm,
    # CONCOCT cuts the assembly into 10 kb fragments and clusters them, so it is the heaviest of the three
    # on a Pratama assembly of over a million contigs. It gets the widest request; 20 h keeps the ladder legal.
    resources=Resources(cpus=32, memory=Size.GB(192), duration=Duration(hours=20)),
)
