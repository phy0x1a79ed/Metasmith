...
| GTDB r232 | **COMPLETE AND USABLE** — the classify_wf blocker is repaired | **Both archives transferred from the lab's chinook Globus collection and extracted.** The reps genomes are at `/scratch/phyberos/staging/gtdb/release232/skani/database/{GCF,GCA}/xxx/yyy/zzz/<acc>_genomic.fna.gz` — **GCF 40,255 + GCA 159,668 = 199,923 genomes**, 224,110 directories, **426,970 inodes measured**, **260 GB** (274 GB for all of release232). Extracted with `--strip-components=2` straight into `skani/database`, because the archive already ships that tree; `create_genome_paths.sh` is a filing tool for FLAT `.gz` and must NOT be run. **The precise failure is verified repaired**: `GCF/000/367/345/GCF_000367345.1_genomic.fna.gz` — the genome whose absence produced `ERROR: Reference genome missing from skani database` after 33m44s of placement work — is present at 1,308,796 bytes, and a sampled genome is a valid gzip carrying real sequence. Path confirmed against GTDB-Tk 2.6.1's own `SKANI_DIR` (`config/common.py:156`); `fastani` is gone from 2.6.1.

    CAUTION this tree is 42% of the entire 1 M inode quota. A squashfs image bound with
    `image-src` would replace all 426,970 with ONE inode, buildable on node-local disk
    (7.2 TB per cpubase node) so it never touches the quota. But `skani/database/` must hold
    BOTH the genome trees AND the four sketch files (`sketches.db` 75.3 GB, `markers.bin`
    9.9 GB, `index.db`, `metadata.tsv`) -- an image bound over the whole directory would hide
    them, so bind `GCF/` and `GCA/` separately or put the sketches inside the image too.
